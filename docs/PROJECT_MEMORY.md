...
|---|---|---|---|---|
| [trefeon/freebuff-proxy](https://github.com/trefeon/freebuff-proxy/commit/7ced74e75df0b21b0c7ff7689dfe6cddd0524fa3) | MIT | `7ced74e75df0b21b0c7ff7689dfe6cddd0524fa3`（2026-08-15） | `banned`/`resumes_at` 冷却、429 的 `retryAfterMs`/`resetAt`/Pacific midnight 优先级 | `worker.js`: `parseCooldown`、`nextPacificMidnight`、`poolExhaustionResponse` |
...
- `banned` 有 `resumes_at` 就用其截止时间；没有则至少隔离 24 小时。
- `token_invalid` 默认不自动过期，必须替换凭据或成功的管理员探测清除。
...
2. 对照官方 `CodebuffAI/freebuff` 的模型/agent 映射，确认 root agent、错误码和 `resumes_at` 字段没有变更。
3. 先把新行为写成失败测试，再决定是否调整 `parseCooldown`、池耗尽分类、revision 或租约。
4. 检查持久文件 schema 兼容、token 是否仍只以哈希落盘、旧快照不会回灌状态、管理员清除是否需要审计日志。
5. 运行 `node test/unit.test.mjs`、`node --test test/account-state.test.mjs`、`node --test test/proxy.test.mjs`、布局测试、语法检查和 `git diff --check`；确认 pre-response abort、错误 schema 备份恢复和写盘失败原子性测试仍通过。
